...
//		primaryfileRepository.deleteAll();
	...
	public void shouldCreatePrimaryfile() throws Exception {
		mockMvc.perform(post("/primaryfiles").header("Authorization", token).content(
	...
	public void shouldRetrievePrimaryfile() throws Exception {
		MvcResult mvcResult = mockMvc.perform(post("/primaryfiles").header("Authorization", token).content(
				"{\"name\": \"Primaryfile 1\", \"description\":\"For test\"}")).andExpect(
	...
	public void shouldQueryPrimaryfileDescription() throws Exception {
		...
		mockMvc.perform(post("/primaryfiles").header("Authorization", token)
				  .content(json)).andExpect(
						  status().isCreated());
		mockMvc.perform(
				get("/primaryfiles/search/findByDescription?description={description}", primaryfile.getDescription()).header("Authorization", token)).andDo(
	...
	public void shouldQueryPrimaryfileCreatedBy() throws Exception {
		...
		mockMvc.perform(
				get("/primaryfiles/search/findByCreatedBy?createdBy={createdBy}", TestHelper.TEST_USER).header("Authorization", token)).andDo(
						MockMvcResultHandlers.print()).andExpect(
						status().isOk()).andExpect(
	...
	public void shouldQueryPrimaryfileName() throws Exception {
		mockMvc.perform(post("/primaryfiles").content(
				"{ \"name\": \"Primaryfile 1\", \"description\":\"For test\"}").header("Authorization", token)).andExpect(
						status().isCreated());

		mockMvc.perform(
				get("/primaryfiles/search/findByName?name={name}", "Primaryfile 1").header("Authorization", token)).andExpect(
						status().isOk()).andExpect(
	...
	public void shouldQueryPrimaryfileNameKeyword() throws Exception {
		...
		mockMvc.perform(
				get("/primaryfiles/search/findByKeyword?keyword={keyword}", keyword).header("Authorization", token)).andDo(
						MockMvcResultHandlers.print()).andExpect(
						status().isOk()).andExpect(
	...
	public void shouldPartiallyUpdatePrimaryfile() throws Exception {
		...
		mockMvc.perform(
				patch(location).header("Authorization", token).content("{\"name\": \"Primaryfile 1.1.1\"}")).andExpect(
						status().isNoContent());
