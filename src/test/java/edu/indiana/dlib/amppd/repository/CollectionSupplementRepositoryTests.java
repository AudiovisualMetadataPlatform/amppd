...
//			.andExpect(jsonPath("$.validationErrors[2].field").value("handleBeforeCreate.collectionSupplement.item"))
//			.andExpect(jsonPath("$.validationErrors[2].message").value("must not be null"));
	...
	public void shouldErrorOnDuplicateCreate() throws Exception {
		...
		mockMvc.perform(post("/collectionSupplements").header("Authorization", token).content(json))
			...
			.andExpect(jsonPath("$.validationErrors[0].message").value("Collection supplement name must be unique within its parent collection"));
	}
